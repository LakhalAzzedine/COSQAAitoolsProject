...
export class DefectAnalyzer {
  private defectCategories: DefectCategory[] = [
    {
  ...
  analyzeDefect(description: string, jiraData?: any): RootCauseAnalysis {
    ...
    const severity = this.determineSeverity(lowercaseDesc, jiraData);
    ...
    return {
      primaryCause: this.identifyPrimaryCause(lowercaseDesc, identifiedCategory),
      contributingFactors,
      category: identifiedCategory.name,
      severity,
      impactAreas,
      preventionMeasures: identifiedCategory.preventionStrategies,
      testingGaps: this.identifyTestingGaps(lowercaseDesc, identifiedCategory),
      recommendedActions: this.generateRecommendedActions(lowercaseDesc, identifiedCategory, severity)
    };
  }
