  ...
  const handleGenerateTests = async () => {
    ...
    try {
      ...
      if (importedFiles.length > 0) {
        ...
        if (jiraData) {
          formData.append('jiraId', jiraData.id || jiraData.key || ''); // Use jiraId instead of jiraData
        ...
        requestBody = JSON.stringify({
          ...
          jiraId: jiraData?.id || jiraData?.key || '' // Include jiraId in JSON payload
        });
      }
    ...
    try {
      ...
      const response = await fetch(endpointUrl, {
        ...
        body: JSON.stringify({
          ...
          jiraId: jiraData?.id || jiraData?.key || '', // Use jiraId instead of jiraData
          toolId: "test-generator"
        })
      });
